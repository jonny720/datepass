...
export function IntroCardsStep({ draft, updateDraft, onNext, onBack }: IntroCardsStepProps) {
  ...
  const allPrompts = getIntroPrompts(language, draft.introTone, draft.inviteType, draft.theme);
  ...
  useEffect(() => {
    if (draft.introCards.length === 0 && showPrompts) {
      const defaultPrompts = getDefaultPrompts(language, draft.introTone, draft.inviteType, draft.theme);
  ...
  }, [showPrompts, draft.introCards.length, language, draft.introTone, draft.inviteType, draft.theme, updateDraft]);
